# tools/build_only_scada_5y_match_scada_cerra_rules_dataset2.py
...
OUT_DIR = "data/wind_onlyscada_no_5b_dataset2"
...
# ===== 多时间尺度设置（与 scada+cerra dataset2 脚本一致）=====
X_STEP_HOURS = 3
Y_STEP_HOURS = 1
H = 6
L = 9

# ===== 年份划分（与 scada+cerra dataset2 脚本一致）=====
...
def tid_to_cerra_tag(tid: int) -> str:
    """
    Map integer turbine id to a tag like T01 / T010
    (不是必须，但写进 meta 方便你之后做映射/核对)
    """
    tid = int(tid)
    if tid >= 10:
        return f"T{tid:03d}"
...
def is_ok_lost(arr: np.ndarray) -> np.ndarray:
    """
    规则（与 scada+cerra dataset2 脚本一致）：
      NaN -> exclude
      0   -> include
      >0  -> exclude
    """
...
def build_adj_and_xy_from_static(static_path, turbine_ids):
    """
    与 build_scada_cerra_v1_s2_FnpOnlyAnalysis_dataset2.py 完全一致的 robust static reader：
    - 自动识别 turbine id 列/名称列
    - 自动识别 lat/lon 列
    - 确保覆盖 turbine_ids 完整，否则报错（避免节点顺序错乱）
    """
    ...
    # adjacency from Gaussian kernel (row-normalized)
...
def _auto_find_static_file(raw_dir: str):
    # 与 scada+cerra dataset2 脚本一致：优先 Penmanshiel_WT_static.csv
    cands = [
...
def main():
    # -------- 收集文件：Dataset2 Penmanshiel pattern --------
    ...
    # -------- 读取所有 SCADA（拼接多文件，过滤到 2016-2020）--------
    turb_raw = {}
    ...
    meta = {
        ...
        "turbine_tags": turbine_tags,           # 额外存一下，方便你核对节点->CERRA tag

        "turbine_latlon": latlon.tolist(),      # 额外存一下（可用于 build_coords）
        "turbine_xy": xy.tolist(),              # 额外存一下（可用于 build_coords）
    ...
    print("N turbines =", N, "Features =", len(feature_names))
