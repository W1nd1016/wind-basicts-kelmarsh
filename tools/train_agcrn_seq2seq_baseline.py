...
from models.agcrn_seq2seq_baseline2 import AGCRNSeq2SeqBaseline
...
def main():
    ...
    meta = json.load(open("data/wind_scada_cerra_4y_resample/meta.json"))
    ...
    train_ds = WindSTFDatasetScadaCerraYearSplit(root="data/wind_scada_cerra_4y_resample", split="train", L=L, H=H)
    val_ds   = WindSTFDatasetScadaCerraYearSplit(root="data/wind_scada_cerra_4y_resample", split="val",   L=L, H=H)
    test_ds  = WindSTFDatasetScadaCerraYearSplit(root="data/wind_scada_cerra_4y_resample", split="test",  L=L, H=H)
    ...
    ckpt = "output/agcrn_seq2seq_baseline_best.pt"
    ...
    for ep in range(1, max_epochs + 1):
        ...
        print(f"[AGCRN-Seq2Seq] Ep {ep:03d} tf={tf_ratio:.2f} | train {train_mae:.4f} | val MAE 3h {val_mae3:.4f} RMSE 3h {val_rmse3:.4f} val MAE 6h {val_mae6:.4f} RMSE 6h {val_rmse6:.4f}")
    ...
    print(f"[TEST][AGCRN-Seq2Seq] MAE 3h {test_mae3:.4f}  RMSE 3h {test_rmse3:.4f} | MAE 6h {test_mae6:.4f} | RMSE 6h {test_rmse6:.4f}")
