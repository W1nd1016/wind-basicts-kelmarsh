# tools/build_scada_cerra_v1_s2_FnpOnlyAnalysis.py
...
OUT_DIR   = "data/wind_scada_cerra_v1_s2_FnpOnlyAnalysis"
...
def _cerra_year_paths(tur_name: str):
    pattern = os.path.join(CERRA_DIR, f"CERRA_75m_{tur_name}_*.csv")
    return sorted(glob.glob(pattern))


def load_neighbors_pos(tid: int):
    """
...
def read_cerra_analysis_all_years(tid: int, full_time_x: pd.DatetimeIndex):
    """
    ...
    """
    tur_name = f"KWF{tid}"
    paths_year = _cerra_year_paths(tur_name)
    if not paths_year:
        raise FileNotFoundError(f"找不到 CERRA 年文件: {os.path.join(CERRA_DIR, f'CERRA_75m_{tur_name}_*.csv')}")
    ...
    if miss:
        raise KeyError(f"[{tur_name}] 缺少 analysis 列: {miss[:6]} ... total_missing={len(miss)}")
...
def read_cerra_forecast_all_years(tid: int, full_time_x: pd.DatetimeIndex):
    """
        ...
        for k in range(1, K_NEIGHBORS + 1):
            for vi, vname in enumerate(CERRA_VAR_ORDER):
                ...
                if col not in df_all.columns:
                    raise KeyError(f"[{tur_name}] 缺少 forecast 列: {col}")
...
def main():
    ...
    for tid in turbine_ids:
        ...
        AN, ANv, an_names = read_cerra_analysis_all_years(tid, full_time_x)  # (T3, K*4)
        ...
        FC, FCv = read_cerra_forecast_all_years(tid, full_time_x)  # (T3,H,K,4)
        FC_list.append(FC)
        FCv_list.append(FCv)

        # --- neighbors pos (dx,dy,dist) normalized ---
        pos, scales = load_neighbors_pos(tid)  # (K,3)
        pos_list.append(pos)
    ...
    FC_feat = np.stack(FC_list, axis=2).astype(np.float32)     # (T3,H,N,K,4) because list is per turbine -> axis=2 is N
