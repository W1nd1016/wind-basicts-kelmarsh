...
# 🔴 改这里：使用「彻底移除 vector 分支」的 FNP
from models.fnp_fusion_OnlyAnalysis_ab_remove_vec_branch import FNPFusion
...
def eval_one(model, loader, device, coords, pos):
    model.eval()
    maes, rmses = [], []
    for x, xv, y, m, fc0, _fc0v in loader:
        # ❗ fc0v 不再使用，但 dataloader 结构不改
        x, xv, y, m, fc0 = (
            x.to(device), xv.to(device), y.to(device),
            m.to(device), fc0.to(device)
        )
...
def main():
    ...
    # ===============================
    # 🔥 Ablation: ONLY scalar forecast
    # ===============================
    fc_emb_scalar = 32
    exog_dim = fc_emb_scalar   # ❗ 不再有 vector
    ...
    agcrn = AGCRNSeq2SeqBaseline(
        ...
        topk=None,
        dropout=0.1,
        exog_dim=exog_dim,   # ✅ 只等于 scalar emb
    ).to(device)
    ...
    ckpt = "output/s2_fnp_agcrn_no_vecbranch_best.pt"
    ...
    for ep in range(1, max_epochs + 1):
        ...
        for x, xv, y, m, fc0, _fc0v in train_ld:
            # ❗ fc0v 忽略
            x, xv, y, m, fc0 = (
                x.to(device), xv.to(device), y.to(device),
                m.to(device), fc0.to(device)
        ...
        print(
            f"[S2][FNP(no-vector-forecast)+AGCRN]"
            f"[E={exog_dim}=scalar-only] "
            f"Ep {ep:03d} tf={tf_ratio:.2f} | "
            f"train {train_mae:.4f} | "
