...
from models.fnp_fusion_OnlyAnalysis_noRot_paper1_plus_v2_wake_downwind import FNPFusionOnlyAnalysis_NoRot_Paper1PlusV2_AbMergeVec
...
def build_coords(meta, L, device):
    ...
    coords = np.concatenate(coords, axis=0)  # (L*N,3)
...
def main():
    ...
    # exog_dim stays 64 in this ablation
    exog_dim = 64

    fusion = FNPFusionOnlyAnalysis_NoRot_Paper1PlusV2_AbMergeVec(
        d_model=d_model,
        K_bg=K,
        use_wake=True,
        use_downwind_penalty=True,
        use_angle_calib=False,
    ).to(device)
    ...
    ckpt = "output/s2_fnp_agcrn_ab_mergevec_best.pt"
    ...
    for ep in range(1, max_epochs + 1):
        ...
        print(f"[AB-MergeVec] Ep {ep:03d} tf={tf_ratio:.2f} | train {train_mae:.4f} | val MAE {val_mae:.4f} RMSE {val_rmse:.4f}")
    ...
    print(f"[TEST][AB-MergeVec] MAE {test_mae:.4f} | RMSE {test_rmse:.4f}")
