# models/fnp_fusion_OnlyAnalysis_ab_no_nacelle_rotation.py
import torch
...
def _safe_softmax(logits: torch.Tensor, dim: int = -1, eps: float = 1e-6) -> torch.Tensor:
    # stable softmax that won't return NaN even if logits contain NaN/Inf or are all -inf
...
class NeuralFourierLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if K > 0:
            ...
            gain = torch.exp(torch.clamp(self.log_gain[:K], -3.0, 3.0)).view(1, 1, K, 1)  # ~[0.05,20] cap
...
class TriDAMFreq(nn.Module):
    ...
    def __init__(self, d_model: int, gate_hidden: int = None, branch_drop_p: float = 0.0):
        ...
        # choose init so softplus(logit_temp)=1.0
...
class FNPFusion(nn.Module):
    ...
    def forward(self, x_obs, x_an, coords, pos, fc0, x_an_valid=None, fc0v=None):
        # one-time warning if raw inputs contain non-finite numbers
        ...
        # ===== vector branch (NO nacelle rotation) =====
        W3 = x_obs[..., 2]
        dir_sin = x_obs[..., 3]
        dir_cos = x_obs[..., 4]

        # SCADA: speed+dir -> (u,v) in earth/met frame
        u_sc, v_sc = _wind_uv_from_speed_dir_sincos(W3, dir_sin, dir_cos)
        x_obs_vec = torch.stack([u_sc, v_sc], dim=-1)                       # (B,L,N,2)

        # Analysis: already provides u,v per neighbor
        an_vec = dyn_an[..., 2:4]                                           # (B,L,N,K,2)
        an_uv_v = dyn_an_v[..., 2:4] if dyn_an_v is not None else None

        # Forecast: already provides u,v per neighbor
