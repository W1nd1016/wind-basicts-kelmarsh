...
class TriDAMSoft(nn.Module):
    ...
    def forward(self, z_obs: torch.Tensor, z_bg: torch.Tensor, z_fc: torch.Tensor) -> torch.Tensor:
        ...
        y_mean = (self.shared(z_obs) + self.shared(z_bg) + self.shared(z_fc)) / 3.0
        g_in = torch.cat([z_obs, z_bg, z_fc, y_mean], dim=-1)
        logits = self.gate(g_in)
        w = torch.softmax(logits, dim=-1)

        y_sel = w[..., 0:1] * z_obs + w[..., 1:2] * z_bg + w[..., 2:3] * z_fc
        z = self.fuse(torch.cat([y_mean, y_sel], dim=-1))
...
class FNPFusion(nn.Module):
    def __init__(
        ...
        # scalar: back to original 3 dims [P3, dP3, W3]
        ...
        # ONLY FiLM (remove additive vec->scalar skip to reduce overfit)
        self.vec_film = VecFiLM(self.d_model, dropout_p=film_dropout_p)

        # start small (sigmoid(-2)≈0.12): let model decide to rely on vectors gradually
    ...
    def forward(self, x_obs, x_an, coords, pos, fc0, x_an_valid=None, fc0v=None):
        ...
        # ===== FiLM modulation (weaker + bounded) =====
