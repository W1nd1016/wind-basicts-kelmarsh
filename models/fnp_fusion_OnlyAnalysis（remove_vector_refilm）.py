# models/fnp_fusion_OnlyAnalysis_ab_no_vecfilm.py
import torch
...
class FNPFusion(nn.Module):
    def __init__(
        ...
        self.tri_vec = TriDAMFreq(self.d_model, branch_drop_p=tri_branch_drop_p)

        # keep these modules, but ablation A1 will NOT use them to modulate
    ...
    def forward(self, x_obs, x_an, coords, pos, fc0, x_an_valid=None, fc0v=None):
        ...
        e_fc_s = self.fc_scalar_enc(fc0_speed, pos, valid=fc0_speed_v)
        z_fc_s0 = self.fc_scalar_pool(e_fc_s)
        z_fc_s = z_fc_s0.unsqueeze(1).expand(B, L, N, self.d_model)

        z_scalar = self.tri_scalar(z_obs=z_obs_s, z_bg=z_bg_s, z_fc=z_fc_s)

        # ===== vector branch (still computed, but NOT used to modulate) =====
        W3 = x_obs[..., 2]
        dir_sin = x_obs[..., 3]
        dir_cos = x_obs[..., 4]
        nac_sin = x_obs[..., 5]
        ...
        fc_vec = torch.stack([f_fc, l_fc], dim=-1)
        ...
        e_fc_v = self.fc_vec_enc(fc_vec, pos, valid=fc_uv_v)
        z_fc_v0 = self.fc_vec_pool(e_fc_v)
        z_fc_v = z_fc_v0.unsqueeze(1).expand(B, L, N, self.d_model)

        z_vec = self.tri_vec(z_obs=z_obs_v, z_bg=z_bg_v, z_fc=z_fc_v)

        # ===== Ablation A1: NO FiLM modulation =====
        z = z_scalar
        z = self.out_norm(z)
        z = _nan_to_num(z, nan=0.0, posinf=0.0, neginf=0.0)

        e_fc = torch.cat([e_fc_s, e_fc_v], dim=-1)
        return z, e_fc
