# models/fnp_fusion_OnlyAnalysis_ab_no_forecast.py
import torch
...
class BiDAMFreq(nn.Module):
    """
    Two-branch frequency-domain fusion: obs + bg (NO forecast branch).
    """
    def __init__(self, d_model: int, gate_hidden: int = None, branch_drop_p: float = 0.0):
        ...
        self.gate = nn.Sequential(
            nn.Linear(3, h),  # [mag_o, mag_b, mag_mean]
            nn.ReLU(),
            nn.Linear(h, 2),
        )
    ...
    def forward(self, z_obs: torch.Tensor, z_bg: torch.Tensor) -> torch.Tensor:
        if z_obs.shape != z_bg.shape:
            raise RuntimeError(f"BiDAMFreq expects same shapes, got obs={tuple(z_obs.shape)} bg={tuple(z_bg.shape)}")
        if z_obs.ndim != 4:
            raise RuntimeError(f"BiDAMFreq expects (B,L,N,D), got {tuple(z_obs.shape)}")
        ...
        mag_m = (mag_o + mag_b) / 2.0

        gate_in = torch.stack([mag_o, mag_b, mag_m], dim=-1)  # (B,N,F,3)
        ...
        if self.training and self.branch_drop_p > 0.0:
            ...
            if trigger.any():
                which = torch.randint(low=0, high=2, size=(B, N), device=z_obs.device)
                mask = torch.ones((B, N, 1, 2), device=z_obs.device, dtype=w.dtype)
...
class VecFiLM(nn.Module):
    ...
    def forward(self, z_vec: torch.Tensor):
        z = _nan_to_num(z_vec, nan=0.0, posinf=0.0, neginf=0.0)
        z = self.dropout(z)
        gb = self.mlp(z)
        gamma, beta = torch.chunk(gb, 2, dim=-1)
        gamma = torch.tanh(gamma)
        beta = torch.tanh(beta)
        gamma = _nan_to_num(gamma, nan=0.0, posinf=0.0, neginf=0.0)
        beta = _nan_to_num(beta, nan=0.0, posinf=0.0, neginf=0.0)
        return gamma, beta


class FNPFusion(nn.Module):
    """
    Ablation A2: remove forecast branch from tri-fusion:
      - scalar fusion: obs + bg only
      - vector fusion: obs + bg only
      - do NOT output horizon-wise exog (e_fc=None)
    """
    def __init__(
        self,
        d_model: int = 128,
        ...
        self.bi_scalar = BiDAMFreq(self.d_model, branch_drop_p=bi_branch_drop_p)

        self.obs_vec_enc = FuncRepVFR(
            in_dim=2, d_model=self.d_model, modes=modes,
            setconv_topk=obs_setconv_topk, setconv_drop=obs_setconv_drop,
            spectral_drop=nfl_spectral_drop
        )
        ...
        self.bi_vec = BiDAMFreq(self.d_model, branch_drop_p=bi_branch_drop_p)
    ...
    def forward(self, x_obs, x_an, coords, pos, fc0, x_an_valid=None, fc0v=None):
        # keep signature; forecast inputs are ignored in this ablation
        if (not self._warned_nonfinite):
            for name, t in [("x_obs", x_obs), ("x_an", x_an), ("coords", coords), ("pos", pos)]:
        ...
        # ===== scalar branch (obs + bg) =====
        x_obs_scalar = x_obs[..., 0:3]
        an_speed = dyn_an[..., 0:1]
        an_speed_v = dyn_an_v[..., 0:1] if dyn_an_v is not None else None

        z_obs_s = self.obs_scalar_enc(x_obs_scalar, coords)
        z_bg_s = self.bg_scalar_enc(an_speed, pos, valid=an_speed_v)
        z_scalar = self.bi_scalar(z_obs=z_obs_s, z_bg=z_bg_s)

        # ===== vector branch (obs + bg) =====
        W3 = x_obs[..., 2]
        dir_sin = x_obs[..., 3]
        dir_cos = x_obs[..., 4]
        nac_sin = x_obs[..., 5]
        ...
        x_obs_vec = torch.stack([f_sc, l_sc], dim=-1)
        ...
        an_vec = torch.stack([f_an, l_an], dim=-1)
        an_uv_v = dyn_an_v[..., 2:4] if dyn_an_v is not None else None

        z_obs_v = self.obs_vec_enc(x_obs_vec, coords)
        z_bg_v = self.bg_vec_enc(an_vec, pos, valid=an_uv_v)
        z_vec = self.bi_vec(z_obs=z_obs_v, z_bg=z_bg_v)

        # ===== FiLM modulation (still kept) =====
        ...
        e_fc = None  # no horizon-wise forecast exog
        return z, e_fc
