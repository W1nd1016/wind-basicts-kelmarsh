# datasets/wind_dataset_scada_cerra_s2.py
import json
import numpy as np
import torch
...
class WindDatasetScadaCerraS2(Dataset):
    """
    S2 Dataset:
      x    : (L,N,F)   history (normalized), invalid forced to 0
      xv   : (L,N,F)   0/1 valid mask
      y    : (H,N)     target power (normalized)
      m    : (H,N)     supervision mask
      fc0  : (H,N,F_fc) forecast issued at t0 for lead=1..H  (normalized in X already)
      fc0v : (H,N,F_fc) forecast valid mask
    """
    def __init__(self, root="data/wind_scada_cerra_v1_s2", split="train", L=None, H=None):
        ...
        s2 = meta["s2"]
        self.fc_leads = [int(x) for x in s2["forecast_leads"]]
        self.fc_idx_by_lead = {int(k): v for k, v in s2["cerra_fc_idx_by_lead"].items()}
        self.fc_feat_dim = len(self.fc_idx_by_lead[self.fc_leads[0]]) if self.fc_leads else 0
    ...
    def __getitem__(self, k):
        ...
        xv = self.Xv[x_idx - self.L + 1 : x_idx + 1]        # (L,N,F)
        ...
        fc0_list = []
        fc0v_list = []
        for lead in self.fc_leads[:self.H]:
            idxs = self.fc_idx_by_lead[lead]
            fc0_list.append(x_last[:, idxs])     # (N,F_fc)
            fc0v_list.append(xv_last[:, idxs])   # (N,F_fc)

        fc0  = np.stack(fc0_list, axis=0)        # (H,N,F_fc)
        fc0v = np.stack(fc0v_list, axis=0)       # (H,N,F_fc)
